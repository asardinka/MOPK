...
// Предположение относительно test11.cpp:
// Формат даты в наших тестовых файлах всегда YYYY-MM-DD.
// Поэтому нет смысла для каждой строки проходить дату циклом,
// искать разделители и определять положение Y, M и D.
// В test11 цифры даты читаются напрямую по известным позициям.
// Остальная логика test10 не меняется, чтобы измерить именно эту оптимизацию.
...
// g++ -std=c++17 -O2 test11.cpp -o build/test11.exe && build/test11.exe
